...
func generateCSRWithExtensions(config TestCSRConfig) ([]byte, error) {
	priv, err := rsa.GenerateKey(rand.Reader, 2048)
	if err != nil {
		return nil, err
	}
...
func TestCSRFormatting(t *testing.T) {
	...
	for _, config := range configs {
		t.Run(config.Name, func(t *testing.T) {
			// Generate CSR
			csrPEM, err := generateCSRWithExtensions(config)
			if err != nil {
			...
			csrOut, err := csr.ParseCSR(p.Bytes)
			if err != nil {
				t.Fatalf("Formatter failed: %v", err)
			}

			formatterOutput, err := csrOut.PrintCSR()
